...
def getAgentStats(LlamadasPorAgente: pd.DataFrame) -> pd.DataFrame:
    ...
    df_concatenado_agentes_unicos = LlamadasPorAgente.groupby('id_igs').agg(
        ...
        wpm_at_mac=('wpm_at_mac', 'mean'),
        wpm_at_price=('wpm_at_price', 'mean'),
        vol_mean=('vol_mean', 'mean'),
        vol_std=('vol_std', 'mean'),
        vol_at_mac=('vol_at_mac', 'mean'),
        vol_at_price=('vol_at_price', 'mean'),
        volume_classification_mac=('volume_classification_mac', lambda x: x.mode().iloc[0] if not x.mode().empty else x.iloc[0]),
        volume_classification_prices=('volume_classification_prices', lambda x: x.mode().iloc[0] if not x.mode().empty else x.iloc[0]),
        velocity_classification_macs=('velocity_classification_macs', lambda x: x.mode().iloc[0] if not x.mode().empty else x.iloc[0]),
        velocity_classification_prices=('velocity_classification_prices', lambda x: x.mode().iloc[0] if not x.mode().empty else x.iloc[0]),
        DATE=('DATE_TIME', 'last'),
