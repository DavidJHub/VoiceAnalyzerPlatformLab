...
from setup.MemorySetupRegrade import download_memory_json
from database.S3Loader import renombrar_archivos_s3, download_audio_files
...
def campaign_setup(path_campania,days_ago=0):
    """
    ...
    if df_inventory.empty:
        ...
        if campaign_parameters is not None:
            matrix_path = path_campania + '/misc/'
            #print(f"Parámetros de la campaña {path_campaña}:{campaign_parameters}")
            if not os.path.exists(matrix_path):
                os.makedirs(matrix_path)
            memory_folder = os.path.join(path_campania, 'memory')
            ...
            if path_campania=='PROME':
                print("RENOMBRANDO ARCHIVOS...")
                renombrar_archivos_s3(S3_PATH,'s3iahub.igs','PROME_2_','PROME2_',days_ago)
            if path_campania=='BR_ESP':
                print("RENOMBRANDO ARCHIVOS...")
                renombrar_archivos_s3(S3_PATH,'s3iahub.igs','BR_ESP_','BRESP_',days_ago)
            for i in PATHS:
                route=download_audio_files(i,S3_PATH,SPONSOR, 's3iahub.igs', path_campania, days_ago)
                print("DESCARGANDO MEMORIA ")
                #download_memory_audios(i,S3_PATH, 's3iahub.igs', path_campania,300)
                print("DESCARGANDO TRANSCRIPTOS ")
                #download_memory_json('documentos.aihub',path_campania, COUNTRY,SPONSOR, path_campania)
            #rename_prome_files(path_campania)
            concatenate_audios_main(path_campania)
            print("AISLANDO AUDIOS VACÍOS")
            move_small_audios(path_campania, size_limit_kb=100)
